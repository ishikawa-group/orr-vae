...
# JSONファイルのパス設定（動的に生成）
BASE_DATA_PATH = "/gs/fs/tga-ishikawalab/wakamiya/ORR_catalyst_generator/ccvae/data"
...
class ConditionalVAE(nn.Module):
    def __init__(self, latent_size=128, condition_dim=1):
        ...
        # ======== エンコーダ========
        # 条件値の非線形変換（エンコーダ用）
        self.enc_label_fc1 = nn.Linear(condition_dim, 16)
        self.enc_label_fc2 = nn.Linear(16, 16)
        self.enc_label_fc3 = nn.Linear(16, 8)
        
        # 畳み込み層
        self.conv1 = nn.Conv2d(4+8, 256, kernel_size=3, stride=1, padding=1)
        ...
        # ======== デコーダ========
        # 条件値の非線形変換（デコーダ用）
        self.dec_label_fc1 = nn.Linear(condition_dim, 16)
        self.dec_label_fc2 = nn.Linear(16, 16)
        self.dec_label_fc3 = nn.Linear(16, 8)

        # 全結合層
        self.dec_fc1 = nn.Linear(latent_size + 8, 256)
    ...
    def encode(self, x, y):
        ...
        # 条件を非線形変換
        y_encoded = self.encode_condition_enc(y)  # [B, 8]
        
        # 条件を空間的に拡張
        y_expanded = y_encoded.view(batch_size, -1, 1, 1).expand(-1, -1, 8, 8)  # [B, 8, 8, 8]
        
        # 入力と条件を結合
        x_cond = torch.cat([x, y_expanded], dim=1)  # [B, 12, 8, 8]
    ...
    def decode(self, z, y):
        ...
        # 条件を非線形変換
        y_encoded = self.encode_condition_dec(y)  # [B, 8]
        
        # 潜在変数と条件を結合
        z_cat = torch.cat((z, y_encoded), dim=1)  # [B, latent_size + 8]
...
def main():
    # 結果保存先を動的に設定
    result_dir = f"/gs/fs/tga-ishikawalab/wakamiya/ORR_catalyst_generator/ccvae/result/iter{ITER}"
