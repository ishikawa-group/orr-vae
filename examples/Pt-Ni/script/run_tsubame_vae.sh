...
if command -v module >/dev/null 2>&1; then
  for mod in ${MODULE_LOADS}; do
    if module load "${mod}"; then
      echo "[Pt-Ni] loaded module: ${mod}"
    else
      echo "[Pt-Ni] warning: failed to load module '${mod}'" >&2
    fi
  done
else
  echo "[Pt-Ni] warning: 'module' command is unavailable; skipping module load" >&2
fi

if [ -f "${VENV_PATH}/bin/activate" ]; then
  # shellcheck disable=SC1090
  source "${VENV_PATH}/bin/activate"
  echo "[Pt-Ni] activated virtualenv: ${VENV_PATH}"
else
  echo "[Pt-Ni] warning: virtualenv not found at ${VENV_PATH}; using system python" >&2
fi
