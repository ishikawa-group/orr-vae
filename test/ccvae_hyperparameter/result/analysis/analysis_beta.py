...
# ベースディレクトリ
base_dir = "/gs/fs/tga-ishikawalab/wakamiya/ORR_catalyst_generator/test/ccvae_hyperparameter/result/beta_size"

# β値のリスト
beta_values = ["0-1", "0-5", "1", "2-5", "5", "10"]
beta_labels = ["β = 0.1", "β = 0.5", "β = 1", "β = 2.5", "β = 5", "β = 10"]
...
for i, (beta, label) in enumerate(zip(beta_values, beta_labels)):
    ...
    for iter_num in range(6):  # 0-5まで
        file_path = os.path.join(base_dir, f"beta_size_{beta}", "data", f"iter{iter_num}_calculation_result.json")
...
plt.title('Minimum Overpotential vs Iteration for Different β Values', fontsize=14)
...
# グラフを保存
output_path = os.path.join(output_dir, "min_overpotential_comparison_beta_size.png")
