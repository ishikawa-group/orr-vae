...
# from ase.utils.structure_comparator import SymmetryEquivalenceCheck  # 削除
...
def load_vae_class():
    ...
    # 一時的にsys.argvを変更してargparseエラーを回避
...
def load_existing_structures(data_dir, iter_names):
    ...
    atoms.numbersも保存して高速化
    """
    existing_structures = []
    existing_numbers = []  # 高速化のためnumbersを別途保存
...
def generate_structures():
    ...
    parser.add_argument("--result_dir", type=str,
                        ...
                        help="結果ディレクトリ（VAEモデルの保存場所）")
    parser.add_argument("--vae_model_path", type=str, default=None,
                        help="学習済みVAEモデルのパス (指定しない場合は自動で設定)")
    parser.add_argument("--overpotential_condition", type=int, choices=[0, 1], default=1,
                        help="過電圧条件 (0: 高過電圧, 1: 低過電圧, default: 1)")
    parser.add_argument("--pt_fraction_condition", type=int, choices=[0, 1], default=1,
                        help="Pt割合条件 (0: 高Pt割合, 1: 低Pt割合, default: 1)")
    parser.add_argument("--latent_size", type=int, default=32,
                        help="潜在変数の次元 (default: 32)")
    parser.add_argument("--existing_iters", type=str, nargs='+', default=None,
                        help="重複チェックに含める既存のiter名のリスト (指定しない場合は自動で設定)")
    args = parser.parse_args()

    # iterに基づいてデフォルト値を設定
    ITER = args.iter
    
    # VAEモデルパスの自動設定（相対パス使用）
    ...
    if args.existing_iters is None:
        args.existing_iters = [f"iter{i}" for i in range(ITER + 1)]  # iter0からiter{ITER}まで
    ...
    print(f"過電圧条件: {args.overpotential_condition} ({'低過電圧' if args.overpotential_condition == 1 else '高過電圧'})")
    print(f"Pt割合条件: {args.pt_fraction_condition} ({'低Pt割合' if args.pt_fraction_condition == 1 else '高Pt割合'})")
    print(f"生成数: {args.num}")
    print(f"潜在変数次元: {args.latent_size}")
    print(f"重複チェック対象iter: {args.existing_iters}")
    print(f"重複チェック方法: atoms.numbers配列比較")
    
    # ConditionalVAEクラスを読み込み
    ConditionalVAE = load_vae_class()
    
    # VAEモデルの初期化と重みの読み込み（2つの条件に対応）
    vae_model = ConditionalVAE(latent_size=args.latent_size, condition_dim=2).to(DEVICE)
    
    # モデルファイルの存在チェック
    if not os.path.exists(args.vae_model_path):
        print(f"エラー: VAEモデルファイルが見つかりません: {args.vae_model_path}")
        return
    ...
    with torch.no_grad():
        while successful_generations < args.num and attempt < max_attempts:
            try:
                ...
                condition = torch.tensor([[args.overpotential_condition, args.pt_fraction_condition]],
                ...
                unique_numbers.append(current_numbers.copy())  # numbersも追加
                ...
                data = {
                    ...
                    "pt_fraction_condition": int(args.pt_fraction_condition),
                ...
                if successful_generations % 1 == 0:  # 1個ごとに表示
    ...
    print(f"重複チェック方法: atoms.numbers配列の完全一致")
