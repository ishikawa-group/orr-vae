...
def generate_structures():
    ...
    """
    # デフォルト値を動的に生成
    default_vae_model_path = f"/gs/fs/tga-ishikawalab/wakamiya/ORR_catalyst_generator/ccvae_2/result/iter{ITER}/final_cvae_iter{ITER}.pt"
    ...
    parser.add_argument("--output_dir", type=str, 
                        default="/gs/fs/tga-ishikawalab/wakamiya/ORR_catalyst_generator/ccvae_2/data",
                        help="出力ディレクトリ")
    ...
    with torch.no_grad():
        while successful_generations < args.num and attempt < max_attempts:
            try:
                ...
                # unique_idを生成（numbersの配列をハッシュ化）
                unique_id = f"iter{next_iter}_{successful_generations + 1}_{hash(tuple(current_numbers))}"
