...
class CatalystOrrDataset(Dataset):
    ...
    def __init__(self, structures_db_paths, overpotentials_json_paths, grid_size=[4, 4, 4], 
        ...
        if self.use_binary_labels:
            # 過電圧の小さいものから順にソート
            sorted_indices = sorted(range(len(self.raw_overpotentials)), 
                                  key=lambda i: self.raw_overpotentials[i])
            
            # 新しいラベル方式: 過電圧が小さい上位N個を1、それ以外を0
            self.targets = [0] * len(self.raw_overpotentials)
            high_performance_count = min(self.top_n_high_performance, len(self.raw_overpotentials))
            
            for i in range(high_performance_count):
                idx = sorted_indices[i]
                self.targets[idx] = 1
            
            # 閾値（高性能触媒の中で最大の過電圧値）を計算
            if high_performance_count > 0:
                threshold_idx = sorted_indices[high_performance_count - 1]
                threshold_overpotential = self.raw_overpotentials[threshold_idx]
            else:
                threshold_overpotential = 0.0

            print(f"二値分類ラベル統計:")
            print(f"  高性能触媒（ラベル1）: {high_performance_count}個")
            print(f"  低性能触媒（ラベル0）: {len(self.targets) - high_performance_count}個")
            print(f"  閾値（高性能触媒の最大過電圧）: {threshold_overpotential:.3f} V")
...
def make_data_loaders_from_json(structures_db_paths, overpotentials_json_paths, 
    ...
    dataset = CatalystOrrDataset(
        ...
        use_binary_labels=use_binary_labels,
        normalize_target=normalize_target,
        top_n_high_performance=top_n_high_performance  # 新しいパラメータを渡す
    )
